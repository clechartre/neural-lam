...
class WeatherDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, dataset_name, split="train", standardize=True, subset=False, batch_size=4
    ):
        super().__init__()
        ...
        self.zarr_files = sorted(glob.glob(os.path.join(sample_dir_path, "data*.zarr")))
        ...
        if subset:
            if constants.eval_datetime is not None and split == "test":
                ...
                for i, file in enumerate(self.zarr_files):
                    ...
                    if (
                        file_datetime_obj
                        <= eval_datetime_obj
                        < file_datetime_obj + timedelta(hours=constants.chunk_size)
                    ):
                        ...
                        position_within_file = int(
                            (eval_datetime_obj - file_datetime_obj).total_seconds()
                            // 3600
                        )
            ...
            start_datetime = (
                self.zarr_files[0].split("/")[-1].split("_")[1].replace(".zarr", "")
            )
        ...
        variables_2d = [
            var for var in constants.param_names_short if not constants.is_3d[var]
        ]
    ...
    def __len__(self):
        num_steps = (
            constants.train_horizon if self.split == "train" else constants.eval_horizon
        )
