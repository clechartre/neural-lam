...
Report bugs as [GitHub issues](https://github.com/MeteoSwiss-APN/neural-lam/issues).
...
Look through the [GitHub issues](https://github.com/MeteoSwiss-APN/neural-lam/issues) for bugs. Anything tagged with "bug" and "help wanted" is open to whoever wants to implement it.

### Implement Features

Look through the  [GitHub issues](https://github.com/MeteoSwiss-APN/neural-lam/issues) for features. Anything tagged with "enhancement" and "help wanted" is open to whoever wants to implement it.

### Write Documentation

Neural-LAM could always use more documentation, whether as part of the official Neural-LAM docs, in docstrings --- or even on the web in blog posts, articles, and such.

### Submit Feedback

The best way to send feedback is to file a [GitHub issue]( https://github.com/MeteoSwiss-APN/neural-lam/issues).
...
Ready to contribute? Here's how to set up `neural-lam` for local development.

1. Fork the [`neural-lam` repo](https://github.com/) on GitHub.
2. Clone your fork locally:

    ```bash
    git clone git@github.com:your_name_here/neural-lam.git
    ```
...
3. Create a virtual environment and install the dependencies:
    ...
    This will create a conda environment named `neural-lam` (change with `-n`) and install the pinned runtime and development dependencies in `requirements/environment.yml`.
...
```bash
pytest tests.test_neural_lam
pytest tests.test_neural_lam/test_feature::test_edge_case
```
...
- `src/neural_lam/`: Source code of the project package.
- `tests/test_neural_lam/`: Unit tests of the project package; run with `pytest`.
...
Neural-LAM uses [Conda](https://docs.conda.io/en/latest/) to manage dependencies. (Also check out [Mamba](https://mamba.readthedocs.io/en/latest/) if you like your package installations fast.) Dependencies are specified in YAML files, of which there are two:
...
By default, a single executable script called neural-lam is provided. It is created when the package is installed. When you call it, the main function (`cli`) in `src/neural_lam/cli.py` is called.

When the package is installed, a executable script named `neural-lam` is created in the bin folder of the active conda environment. Upon calling this script in the shell, the `main` function in `src/neural_lam/cli.py` is executed.
