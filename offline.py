...
def main():
    ...
    ds = xr.open_zarr(fsspec.get_mapper("/users/clechart/clechart/neural-lam/data/offline/data_2020011017.zarr", anon=True), consolidated=True).isel(time=1)
    tqv = ds['TQV']
    ...
    save_path = "/users/clechart/clechart/neural-lam/"
    
    # Verify inference with the target dataset
    verify_inference(tqv, file_path=path_prediction_file, save_path=save_path, feature_channel=24)

# Verification function to plot predictions and target data
@matplotlib.rc_context(utils.fractional_plot_bundle(1))
def verify_inference(target, file_path: str, save_path: str, feature_channel: int, vrange=None):
    ...
    fig, axes = plt.subplots(2, 1, figsize=constants.FIG_SIZE, subplot_kw={"projection": constants.SELECTED_PROJ})

    # Plot each dataset
    for ax, data in zip(axes, (target_feature_array, prediction_array)):
        contour_set = ax.contourf(lon, lat, data, transform=constants.SELECTED_PROJ, cmap="plasma", levels=np.linspace(vmin, vmax, num=100))
        ax.add_feature(cf.BORDERS, linestyle="-", edgecolor="black")
        ax.add_feature(cf.COASTLINE, linestyle="-", edgecolor="black")
        ax.gridlines(crs=constants.SELECTED_PROJ, draw_labels=False, linewidth=0.5, alpha=0.5)

    # Add colorbar and titles
    cbar = fig.colorbar(contour_set, orientation="horizontal", aspect=20)
    cbar.ax.tick_params(labelsize=10)
    plt.savefig(os.path.join(save_path, "plot_offline.png"), bbox_inches="tight")
    plt.close(fig)
